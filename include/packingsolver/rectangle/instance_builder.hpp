...
public:
     ...
     *
     * Use this when building a rectangle sub-instance from an instance of a
     * different problem type (so 'set_parameters()', which only copies a
     * same-type 'Parameters', can't be used to propagate it) - pass the
