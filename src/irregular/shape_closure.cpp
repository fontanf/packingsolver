...
Shape close_inflated_elements(const std::vector<ShapeElement>& inflated_elements)
{
    ...
    for (size_t i = 0; i < closed_elements.size() - 1; ++i) {
        ...
        if (euclidean_distance(current.end, next.start) > 1e-6) {
            ...
            if (need_connector) {
                ...
                if (current_length > 0 && next_length > 0) {
                    ...
                    if (mid_length > 0.01) {  // Ensure the middle vector is valid
                        ...
                        LengthDbl angle_between = std::acos(current_dx * next_dx + current_dy * next_dy);
                        ...
                        if (radius < chord_length / 2) radius = chord_length / 2;
                        if (radius > chord_length * 5) radius = chord_length * 5;
