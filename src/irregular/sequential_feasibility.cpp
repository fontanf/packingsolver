        ...
        const SequentialFeasibilityParameters& parameters)
{
    ...
    for (Counter it = 0;; ++it) {
        ...
        if (instance.objective() == Objective::OpenDimensionX) {
            x = 0.99 * (solution.x_max() - bin_type.aabb.x_min) + bin_type.item_bin_minimum_spacing;
        } else if (instance.objective() == Objective::OpenDimensionY) {
            y = 0.99 * (solution.y_max() - bin_type.aabb.y_min) + bin_type.item_bin_minimum_spacing;
        } else {  // OpenDimensionXY
            x = 0.99 * (std::max)(
