        ...
        const MilpParameters& parameters)
{
    ...
    if (solution.empty()) {
        // Timed out before finding any solution; leave 'result.status' at
        // its default 'Inconclusive' - never sound to report 'infeasible'
        // without a proof, nor 'feasible' without an actual solution.
        return result;
    }
    for (size_t candidate_id = 0; candidate_id < candidates.size(); ++candidate_id) {
